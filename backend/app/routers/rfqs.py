...
async def create_rfq(rfq_in: RFQCreate, current_user: User = Depends(get_current_user), db: AsyncSession = Depends(get_db)):
    ...
    db.add(rfq)
    await db.flush() # CRITICAL: This populates rfq.id
    ...
    await db.commit()
    # We don't need to refresh if we just want to return the data we already have
    # rfq.id is populated after commit if it's a UUID generated by the app or DB
    # If it's DB generated, we might need one refresh, but let's try to avoid it.
    await db.refresh(rfq) 
